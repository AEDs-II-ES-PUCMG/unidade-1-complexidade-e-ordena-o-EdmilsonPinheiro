...
public class App {
    ...
    */
    ...
    static void localizarPedidosPremium() {
        ...
        int left = 0;
        int right = quantPedidos - 1;
        int index = quantPedidos; 

        while (left <= right) {
            int mid = left + (right - left) / 2;
            if (pedidosOrdenadosPorValor[mid].valorFinal() >= valorCorte) {
                index = mid;
                right = mid - 1;
            } else {
                left = mid + 1;
        ...
        if (index == quantPedidos) {
            System.out.println("Nenhum pedido encontrado com valor final >= R$ " + String.format("%.2f", valorCorte));
        } else {
            System.out.println("Pedidos premium (valor final >= R$ " + String.format("%.2f", valorCorte) + "):");
            for (int i = index; i < quantPedidos; i++) {
                System.out.println(String.format("%02d - %s\n", (i - index + 1), pedidosOrdenadosPorValor[i].toString()));
            }
    ...
    */
    static void ordenarPedidos(){
        ...
        if (ordenador != null) {
            ...
            System.out.println("Tempo gasto com a ordenação dos pedidos: " + ordenador.getTempoOrdenacao() + " milissegundos");
        }
        ordenador = null;
    }
